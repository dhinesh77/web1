 ...
 */
class ABNewsletterWidget extends WP_Widget {

    function __construct() {
        parent::__construct(false, $name = 'AB Newsletter', array('description' => 'Newsletter widget to add subscription forms on sidebars'), array('width' => '350px'));
    }
    ...
    static function get_widget_form() {
        ...
        if ($options_profile['name_status'] == 2)
            $form .= '<div><input class="newsletter-firstname" type="text" name="nn" value="' . esc_attr($options_profile['name']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></div>';

        if ($options_profile['surname_status'] == 2)
            $form .= '<div><input class="newsletter-lastname" type="text" name="ns" value="' . esc_attr($options_profile['surname']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></div>';

        $form .= '<div class="ab-newsletter-email"><input class="newsletter-email" type="email" required name="ne" value="' . esc_attr($options_profile['email']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></div>';

        if (isset($options_profile['sex_status']) && $options_profile['sex_status'] == 2) {
            $form .= '<div><select name="nx" class="newsletter-sex">';
        ...
        if ($options_profile['privacy_status'] == 1) {
            if (!empty($options_profile['privacy_url'])) {
                $form .= '<div><input type="checkbox" name="ny"/>&nbsp;<a target="_blank" href="' . $options_profile['privacy_url'] . '">' . $options_profile['privacy'] . '</a></div>';
            }
            else
                $form .= '<div><input type="checkbox" name="ny"/>&nbsp;' . $options_profile['privacy'] . '</div>';
        }

        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            $form .= '<div class="ab-newsletter-submit"><input class="newsletter-submit secondary-background" type="image" src="' . $options_profile['subscribe'] . '"/></div>';
        } else {
            $form .= '<div class="ab-newsletter-submit"><input class="newsletter-submit secondary-background" type="submit" value="' . $options_profile['subscribe'] . '"/></div>';
        }
    ...
    function widget($args, $instance) {
        ...
        if (stripos($instance['text'], '<form') === false) {
            ...
            $form .= ABNewsletterWidget::get_widget_form();
            $form .= '</div>';
...
add_action('widgets_init', create_function('', 'return register_widget("ABNewsletterWidget");'));
?>
